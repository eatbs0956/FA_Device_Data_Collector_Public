...
| **设备管理** | 设备列表、设备详情（含协议配置）、批量导入 | el-tree, el-table, el-drawer, el-form | 设备CRUD、分组管理、协议参数配置、连接测试 | Device.* |
...
│  ══════════════ 基本信息 ══════════════              │
│                                                     │
│  设备名称: [________________]  设备编码: [__________] │
│  设备分组: [下拉选择______▼]  协议类型: [下拉选择___▼] │
│  描    述: [______________________________________] │
│  安装位置: [________________]  启用状态: [●启用 ○禁用] │
│                                                     │
├─────────────────────────────────────────────────────┤
│  ══════════════ 连接配置 ══════════════              │
│                                                     │
│  IP 地 址: [________________]  端口号: [_____]       │
│  连接超时: [___] 秒              重试次数: [___]      │
│  心跳间隔: [___] 秒              [测试连接]           │
│                                                     │
├─────────────────────────────────────────────────────┤
...
│                                                     │
│  ┌─ OPC UA ──────────────────────────────────────┐  │
│  │ 终结点URL: [opc.tcp://________________]        │  │
│  │ 安全策略: [None/Basic256Sha256___▼]           │  │
│  │ 安全模式: [None/Sign/SignAndEncrypt▼]         │  │
│  │ 认证方式: [Anonymous/UserName/Cert▼]          │  │
│  │ 用户名:   [____________]  密码: [************] │  │
│  └────────────────────────────────────────────────┘  │
│                                                     │
│  ┌─ Modbus TCP ──────────────────────────────────┐  │
│  │ 从站地址(Unit ID): [1-247__]                   │  │
│  │ 字节序:  [Big Endian▼]   字序: [High-Low▼]    │  │
│  └────────────────────────────────────────────────┘  │
│                                                     │
│  ┌─ Modbus RTU ──────────────────────────────────┐  │
│  │ 从站地址(Slave ID): [1-247__]                  │  │
...
│  ══════════════ 扩展信息 ══════════════              │
│                                                     │
│  厂    商: [________________]  型    号: [__________]│
│  安装日期: [日期选择器____]    备    注: [__________] │
│                                                     │
└─────────────────────────────────────────────────────┘
```

**设计说明**：
- **基本信息区**：设备名称、编码、分组、协议类型等核心字段
- **连接配置区**：IP地址、端口、超时等通用连接参数
- **协议配置区**：根据所选协议类型**动态显示**对应的配置表单
  - 使用`v-if/v-show`条件渲染不同协议的配置组件
  - 每种协议有独立的字段集，存储到设备的`ProtocolConfig` JSONB字段
- **扩展信息区**：可选的元数据信息（厂商、型号等）
...
采集节点（Edge Node）管理是系统配置的核心功能之一，支持分布式采集架构下的节点注册、监控和配置管理。

#### 10.2.1 节点注册方式

| 注册类型 | 触发方式 | 典型场景 | 处理逻辑 |
|---------|---------|---------|---------|
| **自动注册 (auto)** | 采集器首次启动时自动向中心注册 | 标准部署流程，采集器配置完成后启动 | 中心服务根据 NodeId 检查是否已存在，不存在则新建记录 |
| **手动注册 (manual)** | 运维人员在管理界面预创建节点 | 预规划部署、离线配置、特殊网络环境 | 运维手动填写所有信息，采集器启动后匹配 NodeId 进行同步 |
...
sequenceDiagram
    ...
    participant Center as 中心服务
    participant DB as 数据库
    
    Note over Collector: 启动时读取配置文件获取 NodeId
    
    Collector->>Center: POST /api/nodes/register {nodeId, ...系统信息}
    Center->>DB: 查询 NodeId 是否存在
    
    alt NodeId 不存在
        Center->>DB: INSERT 新节点 (registrationType='auto')
        Center-->>Collector: 201 Created {nodeId}
    else NodeId 已存在
        Center->>DB: UPDATE 系统信息字段 (platform, version, ip...)
        Center->>DB: UPDATE lastHeartbeat = NOW()
        Center-->>Collector: 200 OK {nodeId}
    end
    
    loop 心跳周期 (默认30s)
        Collector->>Center: PUT /api/nodes/{nodeId}/heartbeat
        Center->>DB: UPDATE lastHeartbeat = NOW()
        Center-->>Collector: 200 OK
    end
```
...
| 状态 | 判定条件 | 显示样式 | 说明 |
|------|---------|---------|------|
| **在线 (Online)** | `isEnabled=true` && `lastHeartbeat` 在 2 个心跳周期内 | 绿色标签 | 采集器正常运行 |
| **离线 (Offline)** | `isEnabled=true` && `lastHeartbeat` 超过 2 个心跳周期 | 灰色标签 | 采集器未响应或网络异常 |
| **未连接 (Not Connected)** | `isEnabled=true` && `lastHeartbeat=NULL` | 黄色标签 | 手动创建的节点，采集器尚未启动 |
| **已禁用 (Disabled)** | `isEnabled=false` | 红色标签 | 管理员手动禁用 |

#### 10.2.4 字段可编辑性规则

根据节点注册类型和连接状态，界面字段具有不同的可编辑性：

**字段分类**：
- **基本信息字段**：nodeName, location, resourceLimits
- **系统信息字段**：platform, version, ipAddress, port, osInfo, hardwareInfo, installPath

**可编辑性矩阵**：

| 场景 | 基本信息字段 | 系统信息字段 | UI 提示文案 |
|------|-------------|-------------|------------|
| 新增节点 (手动) | ✅ 可编辑 | ✅ 可编辑 | "手动创建节点时可预填所有信息" |
| 编辑自动注册节点 | ✅ 可编辑 | ❌ 禁用 (灰色) | "系统信息由采集器自动上报，不可手动修改" |
| 编辑手动节点 (未连接) | ✅ 可编辑 | ✅ 可编辑 | "采集器连接前可编辑所有信息，连接后系统信息将被锁定" |
| 编辑手动节点 (已连接) | ✅ 可编辑 | ❌ 禁用 (灰色) | "采集器已连接，系统信息由采集器上报维护" |

**判定逻辑**：
```typescript
// 前端判定逻辑
const isManualNode = model.registrationType === 'manual';
const hasConnected = model.lastHeartbeat !== null;
const canEditSystemFields = isManualNode && !hasConnected;
```
...
| API | 方法 | 描述 | 请求参数 | 响应格式 | 权限要求 |
|-----|------|------|----------|----------|----------|
| `/api/edge-nodes` | GET | 节点列表 | `?keyword&isEnabled&page&pageSize` | `{items[], total}` | Node.Read |
| `/api/edge-nodes` | POST | 创建节点 | `{nodeId, nodeName, ...}` | `{id, nodeId, ...}` | Node.Write |
| `/api/edge-nodes/{id}` | GET | 节点详情 | 无 | `{id, nodeId, ...}` | Node.Read |
| `/api/edge-nodes/{id}` | PUT | 更新节点 | `{nodeName, location, ...}` | `{success}` | Node.Write |
| `/api/edge-nodes/{id}` | DELETE | 删除节点 | 无 | `{success}` | Node.Delete |
| `/api/edge-nodes/{nodeId}/heartbeat` | PUT | 心跳上报 | `{timestamp}` | `{success}` | Node.Heartbeat |
| `/api/edge-nodes/{nodeId}/register` | POST | 节点注册 | `{platform, version, ...}` | `{id, nodeId}` | Node.Register |
