...
function getSubmitParams() {
  ...
  // 只提取后端 MenuDto 需要的字段
  const submitParams = {
    menuType: Number.parseInt(params.menuType, 10),
    menuName: params.menuName,
    routeName: params.routeName,
    routePath,
    component,
    i18nKey: params.i18nKey || null,
    icon: params.icon || '',
    iconType: params.iconType,
    parentId: params.parentId === 0 ? null : params.parentId,
    order: params.order || 0,
    status: Number.parseInt(params.status || '1', 10),
...
async function handleSubmit() {
  try {
    ...
    if (props.operateType === 'edit' && props.rowData?.id) {
      // 编辑菜单
      const { error } = await fetchUpdateMenu(props.rowData.id.toString(), params);
      if (error) {
      ...
      if (error) {
        window.$message?.error(error.message || '添加失败');
        return;
      }
